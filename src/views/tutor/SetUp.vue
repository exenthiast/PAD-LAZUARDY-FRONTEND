<template>
  <div class="min-h-screen bg-gray-50 text-gray-800 pb-24 relative px-4 md:px-8">
    <!-- 🌟 Splash / Popup -->
    <transition name="fade-scale">
      ...
      >
        <div class="bg-white p-6 rounded-2xl shadow-lg max-w-md text-center relative animate-fadeIn">
          <button
            @click="closeSplash"
          ...
          <p class="text-sm text-gray-600 leading-relaxed">
            Sambil menunggu proses verifikasi oleh admin, yuk lengkapi dan percantik
            <b>profil publikmu</b>. Profil yang menarik akan membantu calon siswa memilih kamu
            sebagai tutor mereka.
          </p>
    ...
    <!-- 🧩 Body -->
    <main class="max-w-4xl mx-auto py-8 space-y-10">
      <!-- Tentang Saya -->
      <section class="bg-white p-6 rounded-xl shadow-sm">
        <div class="flex justify-between items-center mb-3">
          <h2 class="text-lg font-semibold text-gray-800">Tentang Saya</h2>
          <button class="text-sm text-primary hover:underline" @click="editSection('about')">
            Edit
          </button>
        </div>
        <p class="text-sm leading-relaxed text-gray-600">
          {{ tutor.about || 'Belum ada deskripsi diri, tambahkan informasi tentang dirimu di sini.' }}
        </p>
      </section>
      ...
      <section class="bg-white p-6 rounded-xl shadow-sm">
        <div class="flex justify-between items-center mb-3">
          <h2 class="text-lg font-semibold text-gray-800">Keahlian & Mata Pelajaran</h2>
          <button class="text-sm text-primary hover:underline" @click="editSection('skills')">
            Edit
          </button>
        </div>
      ...
      <section class="bg-white p-6 rounded-xl shadow-sm">
        <div class="flex justify-between items-center mb-3">
          ...
          <button class="text-sm text-primary hover:underline" @click="editSection('schedule')">
            Edit
          </button>
        </div>
        <div class="divide-y divide-gray-100 border rounded-lg overflow-hidden">
          ...
          >
            <div>
              <div class="text-sm font-medium text-gray-800">{{ item.day }}</div>
              <div class="text-xs text-gray-500">{{ item.subject }}</div>
            </div>
            <div class="text-sm text-gray-600">{{ item.time }}</div>
          </div>
          <p v-if="!tutor.schedule.length" class="text-sm text-gray-500 px-4 py-3">
      ...
      <section class="bg-white p-6 rounded-xl shadow-sm">
        <div class="flex justify-between items-center mb-3">
          <h2 class="text-lg font-semibold text-gray-800">Pendidikan & Pengalaman</h2>
          <button class="text-sm text-primary hover:underline" @click="editSection('cv')">
            Edit
          </button>
        </div>
        <div v-for="(cv, i) in tutor.cvList" :key="i" class="py-2 border-b last:border-0">
